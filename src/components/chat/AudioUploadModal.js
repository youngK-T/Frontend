...
export default function AudioUploadModal({ isOpen, onClose }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 업로드 완료 후 meetings 페이지로 이동
      if (result.scriptId) {
        alert('회의 분석이 완료되었습니다! 회의 목록 페이지로 이동합니다.');
        onClose();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">새 회의 분석</h2>
